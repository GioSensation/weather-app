...
var _weatherModel = __webpack_require__(133);

var _cityPresenter = __webpack_require__(337);

document.querySelector('.js-search-city').addEventListener('click', searchCity);

function searchCity() {
    var textField = document.querySelector('.js-city-field');
    var city = textField.value;
    (0, _weatherModel.fetchWeather)(city).then(function (response) {
        (0, _cityPresenter.renderCity)(response);
    });
    textField.value = '';
}
...
var WEATHER_API_KEY = exports.WEATHER_API_KEY = '4d202f755d17e552c655cea328f2cf7a';

var DATE_FORMATTER = exports.DATE_FORMATTER = function DATE_FORMATTER(date) {
...
var _endpoints = __webpack_require__(134);

function _asyncToGenerator(fn) { return function () { var gen = fn.apply(this, arguments); return new Promise(function (resolve, reject) { function step(key, arg) { try { var info = gen[key](arg); var value = info.value; } catch (error) { reject(error); return; } if (info.done) { resolve(value); } else { return Promise.resolve(value).then(function (value) { step("next", value); }, function (err) { step("throw", err); }); } } return step("next"); }); }; }

var todaysWeatherUrl = (0, _endpoints.getOpenWeatherMapUrl)('weather')({ units: 'metric' });
var forecastUrl = (0, _endpoints.getOpenWeatherMapUrl)('forecast')({ units: 'metric', cnt: 16 });
...
var getTodaysWeather = function () {
        ...
        return regeneratorRuntime.wrap(function _callee$(_context) {
            while (1) {
                switch (_context.prev = _context.next) {
                    case 0:
                        return _context.abrupt('return', fetch(todaysWeatherUrl(city)).then(function (response) {
    ...
    return function getTodaysWeather(_x) {
        ...
        return regeneratorRuntime.wrap(function _callee2$(_context2) {
            while (1) {
                switch (_context2.prev = _context2.next) {
                    case 0:
                        return _context2.abrupt('return', fetch(forecastUrl(city)).then(function (response) {
                            return response.json();
                        }));
...
var fetchWeather = exports.fetchWeather = function () {
        ...
        return regeneratorRuntime.wrap(function _callee3$(_context3) {
            while (1) {
                switch (_context3.prev = _context3.next) {
                    case 0:
                        _context3.next = 2;
                        return Promise.all([getTodaysWeather(city), getWeatherForecast(city)]);
                    ...
                    case 2:
                        ...
                        today = _ref5[0];
                        forecast = _ref5[1];
                        return _context3.abrupt('return', { today: today, forecast: forecast });
    ...
    return function fetchWeather(_x3) {
...
exports.getOpenWeatherMapUrl = exports.getParamString = undefined;

var _const = __webpack_require__(132);

var getParamString = exports.getParamString = function getParamString() {
...
var getOpenWeatherMapUrl = exports.getOpenWeatherMapUrl = function getOpenWeatherMapUrl(endpoint) {
    return function (params) {
        return function (city) {
            return 'https://api.openweathermap.org/data/2.5/' + (endpoint + '?q=' + city + '&appid=' + _const.WEATHER_API_KEY) + getParamString(params);
        };
    };
};
...
if (!(TO_PRIMITIVE in proto)) __webpack_require__(11)(proto, TO_PRIMITIVE, __webpack_require__(139));
...
var _parser = __webpack_require__(338);

var renderForecast = function renderForecast(forecastTemplate, firstIndex, lastIndex) {
    var tableMarkup = forecastTemplate(firstIndex, lastIndex);
    var containerNode = document.querySelector('.js-forecast');
    if (containerNode) containerNode.innerHTML = tableMarkup;
};

var renderCity = exports.renderCity = function renderCity(_ref) {
    var today = _ref.today,
        forecast = _ref.forecast;

    var domElement = document.querySelector('.js-city-weather');
    var todayData = (0, _parser.getTodaysTemplate)(today);
    var firstPage = 0;
    var numOfPages = 5;

    domElement.innerHTML = '\n        ' + todayData + '\n        <button class="up-button js-up">Up</button>\n        <div class="js-forecast"></div>\n        <button class="down-button js-down">Down</button>';

    var forecastTemplate = (0, _parser.getForecastTable)(forecast);
    var wrappedRenderForecast = function wrappedRenderForecast() {
        return renderForecast(forecastTemplate, firstPage, firstPage + numOfPages - 1);
    };
...
exports.getForecastTable = exports.getForecastRow = exports.getTodaysTemplate = exports.getSunriseSunset = exports.getTodaysTemperatures = exports.getWeatherData = exports.getCountry = exports.getCity = undefined;

var _timezone = __webpack_require__(339);

var getCity = function getCity(response) {
    return response.name;
};
var getCountry = function getCountry(response) {
    return response.sys.country;
};
var getWeatherData = function getWeatherData(response) {
    var _response$weather$ = response.weather[0],
        main = _response$weather$.main,
        description = _response$weather$.description;

    return main + ' (' + description + ')';
};
var getTodaysTemperatures = function getTodaysTemperatures(response) {
    var _response$main = response.main,
        temp = _response$main.temp,
        temp_min = _response$main.temp_min,
        temp_max = _response$main.temp_max;

    return 'Temperature (min, average, max): ' + [temp_min, temp, temp_max].join(', ');
};

var getSunriseSunset = function getSunriseSunset(_ref) {
    var _ref$sys = _ref.sys,
        sunrise = _ref$sys.sunrise,
        sunset = _ref$sys.sunset,
        timezone = _ref.timezone;
    return 'Sunrise: ' + (0, _timezone.getTime)(sunrise, timezone) + ', Sunset: ' + (0, _timezone.getTime)(sunset, timezone);
};

var getTodaysTemplate = function getTodaysTemplate(response) {
...
var getForecastRow = function getForecastRow(forecast) {
    return function (rowNumber) {
        var row = forecast.list[rowNumber];
        if (!row) return '';

        var date = (0, _timezone.getDate)(row.dt, forecast.city.timezone);
        return ('\n<tr>\n    <td>' + date + '</td>\n    <td>' + row.main.temp + '</td>\n    <td>' + row.weather[0].main + ' (' + row.weather[0].description + ')</td>\n</tr>\n').trim();
    };
};

var getForecastTableBody = function getForecastTableBody(rowTemplateFunction, currentRow, lastRow, accumulator) {
    return currentRow > lastRow ? accumulator : getForecastTableBody(rowTemplateFunction, currentRow + 1, lastRow, accumulator + '\n' + rowTemplateFunction(currentRow));
};

var getForecastTable = function getForecastTable(forecast) {
    return function (firstRow, lastRow) {
        var rowTemplate = getForecastRow(forecast);
        var tableBody = getForecastTableBody(rowTemplate, firstRow, lastRow, '');
...
exports.getTodaysTemperatures = getTodaysTemperatures;
exports.getSunriseSunset = getSunriseSunset;
exports.getTodaysTemplate = getTodaysTemplate;
exports.getForecastRow = getForecastRow;
exports.getForecastTable = getForecastTable;
...
var getTzTimestamp = function getTzTimestamp(timestamp, timezoneOffset) {
    return new Date((timestamp + timezoneOffset) * 1000);
};

var getTime = exports.getTime = function getTime(timestamp, timezoneOffset) {
    var tzTimestamp = getTzTimestamp(timestamp, timezoneOffset);
    // Get UTC timestamps and add 0 if needed to have ie. '05' from '5'
    var hours = ('' + tzTimestamp.getUTCHours()).padStart(2, 0);
    var minutes = ('' + tzTimestamp.getUTCMinutes()).padStart(2, 0);
    return hours + ':' + minutes;
};

var getDate = exports.getDate = function getDate(timestamp, timezoneOffset) {
    var tzTimestamp = getTzTimestamp(timestamp, timezoneOffset);
    return (0, _const.DATE_FORMATTER)(tzTimestamp);
};
